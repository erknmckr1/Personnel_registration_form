    ...
    useEffect(() => {
      const getData = async () => {
        try {
          ...
          setSelectedOrder(res.data.cila_work_table[res.data.cila_work_table.length -1])
          if (res.status === 200) {
  ...
  //  const selectOrder = orderTable && lastProcess ? { ...filteredOrder[0], ...lastProcess } : null;
           ...
           setOrderInfo(selectedOrder && orderTable && orderTable.find((item,index)=>item.order_no === selectedOrder.order_no))
       };
